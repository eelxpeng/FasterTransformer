...
template<typename T>
struct AgmTritonModel: public AbstractTransformerModel {
    AgmTritonModel(INIReader reader, std::string model_dir);

    AgmTritonModel(size_t      tensor_para_size,
                   size_t      pipeline_para_size,
                   int         enable_custom_all_reduce,
                   std::string model_dir,
                   int         int8_mode);

    ~AgmTritonModel() = default;
...
private:
    ...
    std::vector<std::shared_ptr<ft::AgmDecodingWeight<T>>> decoding_shared_weights_;

    // agm structure difference
    bool                      agm_with_bias_;

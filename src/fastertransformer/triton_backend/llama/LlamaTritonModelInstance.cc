    ...
    std::shared_ptr<std::unordered_map<std::string, triton::Tensor>> input_tensors)
{
    ...
    if (input_tensors->find("no_repeat_ngram_size") != input_tensors->end()) {
        if (input_tensors->at("no_repeat_ngram_size").shape.size() == 1) {
            ...
            if (ngram > 0) {
                ...
                ft_input_tensors.insert({"no_repeat_ngram_size",
    ...
    if (input_tensors->find("rotary_position_interpolation_factor") != input_tensors->end()) {
        const auto scaling_factor = ((float*)input_tensors->at("rotary_position_interpolation_factor").data)[0];
        ft_input_tensors.insert(
                {"rotary_position_interpolation_factor",
                 {ft::MemoryType::MEMORY_CPU, ft::DataType::TYPE_FP32, {1}, &scaling_factor}});
    }

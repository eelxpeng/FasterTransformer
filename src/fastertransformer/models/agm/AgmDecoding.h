...
template<typename T>
class AgmDecoding: public BaseLayer {
private:
    // meta data
    ...
    constexpr static float layernorm_eps_ = 1e-5f;
    ...
    AgmDecoder<T>*        decoder_;
    AgmContextDecoder<T>* context_decoder_;
...
protected:
    ...
    bool is_allocate_buffer_forced_input_ = false;
    T* input_attention_mask_;
    // T* cross_attention_mask_;
...
public:
    AgmDecoding(size_t                              max_batch_size,
                size_t                              max_seq_len,
    ...
    AgmDecoding(AgmDecoding<T> const& AgmDecoding);
    ...
    void forward(TensorMap* output_tensors, TensorMap* input_tensors, const AgmDecodingWeight<T>* Decoding_weights);

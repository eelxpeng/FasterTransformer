...
#include "src/fastertransformer/models/agm/AgmDecoder.h"

namespace fastertransformer {

template<typename T>
void AgmDecoder<T>::initialize()
{
...
template<typename T>
void AgmDecoder<T>::allocateBuffer()
{
...
template<typename T>
void AgmDecoder<T>::allocateBuffer(size_t batch_size)
{
...
template<typename T>
bool AgmDecoder<T>::isValidBatchSize(size_t batch_size)
{
...
template<typename T>
AgmDecoder<T>::AgmDecoder(size_t                              max_batch_size,
                          size_t                              head_num,
...
template<typename T>
AgmDecoder<T>::AgmDecoder(AgmDecoder<T> const& decoder):
...
template<typename T>
void AgmDecoder<T>::setStream(cudaStream_t stream)
{
...
template<typename T>
bool AgmDecoder<T>::isValidLayerParallelId(uint l)
{
...
template<typename T>
bool AgmDecoder<T>::isFirstLayerParallelId(uint l)
{
    int local_num_layer = (int)(ceil(num_layer_ * 1.0f / pipeline_para_.world_size_));
    return l < num_layer_ && (l == local_num_layer * pipeline_para_.rank_);
}
...
template<typename T>
bool AgmDecoder<T>::isLastLayerParallelId(uint l)
{
    int local_num_layer = (int)(ceil(num_layer_ * 1.0f / pipeline_para_.world_size_));
...
template<typename T>
int AgmDecoder<T>::getFirstLayerParallelId()
{
    int local_num_layer = (int)(ceil(num_layer_ * 1.0f / pipeline_para_.world_size_));
...
template<typename T>
void AgmDecoder<T>::forward(TensorMap*                    output_tensors,
                            ...
                            const std::vector<AgmDecoderLayerWeight<T>*>* decoder_layer_weight)
{
    ...
    //      relative_attention_bias [num_layers, head_num, step, step] or [num_layers, head_num, max_seq_len,
    //      max_seq_len] ite [1] on cpu cache_indirection [local_batch_size / beam_width, beam_width, max_seq_len]
    ...
    const uint     ite             = input_tensors->at("pipeline_iteration").getVal<uint>();
    ...
    for (uint l = 0; l < num_layer_; l++) {
        ...
        for (auto t = output_tensors->at("key_mem_cache").shape.begin() + 2; t != output_tensors->at("key_mem_cache").shape.end(); ++t) {
            ite_cache_offset *= *t;
        }
        ...
        self_attention_input_tensors.insert("input_query", Tensor{MEMORY_GPU, data_type, {local_batch_size, d_model_}, decoder_normed_input_});

        //Inject Necessary information for on the fly Relative position Bias computation
        int num_bucket = input_tensors->at("num_bucket").getVal<int>();
        self_attention_input_tensors.insert(
            "relative_attention_bias",
            Tensor{MEMORY_GPU,
                   data_type,
                   // Shape is (Local Heads, Num Buckets) - we will point to the correct layer here
                   {head_num_ / tensor_para_.world_size_, num_bucket},
        ...
        TensorMap ffn_input_tensors(
            {{"ffn_input", Tensor{MEMORY_GPU, data_type, {local_batch_size, d_model_}, normed_self_attn_output_}}});
        if (has_ia3) {
        ...
        if (use_moe) {
            ...
            invokeT5AddBiasResidual(decoder_output, self_attn_output_, ffn_bias, local_batch_size, d_model_, stream_);
        }
        sync_check_cuda_error();
...
template class AgmDecoder<float>;
template class AgmDecoder<half>;
#ifdef ENABLE_BF16
template class AgmDecoder<__nv_bfloat16>;
#endif

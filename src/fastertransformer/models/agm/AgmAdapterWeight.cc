...
#include "src/fastertransformer/models/agm/AgmAdapterWeight.h"
...
template<typename T>
AgmAdapterWeight<T>::AgmAdapterWeight(const size_t d_model,
...
{
    FT_LOG_DEBUG("AgmAdapterWeight " + std::string(__func__) + " start");
    initialize();
    mallocWeights();
    setWeightPtr();
    FT_LOG_DEBUG("AgmAdapterWeight " + std::string(__func__) + " end");
}

template<typename T>
void AgmAdapterWeight<T>::initialize()
{
    FT_LOG_DEBUG("AgmAdapterWeight " + std::string(__func__) + " start");
...
template<typename T>
AgmAdapterWeight<T>::~AgmAdapterWeight()
{
    FT_LOG_DEBUG("AgmAdapterWeight " + std::string(__func__) + " start");
...
template<typename T>
AgmAdapterWeight<T>::AgmAdapterWeight(const AgmAdapterWeight& other):
...
template<typename T>
AgmAdapterWeight<T>& AgmAdapterWeight<T>::operator=(const AgmAdapterWeight<T>& other)
{
    if (this == &other) {
        return *this;
    }
...
template<typename T>
void AgmAdapterWeight<T>::setWeightPtr()
{
    FT_LOG_DEBUG("AgmAdapterWeight " + std::string(__func__) + " start");
    if (enabled()) {
...
template<typename T>
void AgmAdapterWeight<T>::mallocWeights()
{
    FT_LOG_DEBUG("AgmAdapterWeight " + std::string(__func__) + " start");
    if (enabled()) {
        for (int i = 0; i < AdapterWeights::kNum; i++) {
...
template<typename T>
void AgmAdapterWeight<T>::loadModel(std::string const& dir_path, FtCudaDataType model_file_type)
{
    FT_LOG_DEBUG("AgmAdapterWeight " + std::string(__func__) + " start");
    ...
    if (enabled()) {
        ...
        loadWeightFromBin<T>(adapter_weights_ptr_[AdapterWeights::kAttentionInput],
                             ...
                             dir_path + "after_attention_adapter.DenseSiluDense.wi.weight.bin",
                             model_file_type);
        loadWeightFromBin<T>(adapter_weights_ptr_[AdapterWeights::kAttentionOutput],
                             ...
                             dir_path + "after_attention_adapter.DenseSiluDense.wo.weight.bin",
                             model_file_type);
...
template struct AgmAdapterWeight<float>;
template struct AgmAdapterWeight<half>;
#ifdef ENABLE_BF16
template struct AgmAdapterWeight<__nv_bfloat16>;
#endif

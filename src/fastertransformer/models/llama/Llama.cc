    ...
    size_t batch_size, size_t beam_width, size_t max_seq_len, size_t max_cache_seq_len, size_t max_input_len)
{
    ...
    const size_t self_cache_size = (num_layer_ / pipeline_para_.world_size_) * batchxbeam * max_cache_seq_len
                                   * (head_num_ + 2* kv_head_num_) * size_per_head_ / tensor_para_.world_size_;
                         ...
                         const LlamaWeight<T>*                        gpt_weights)
{
    ...
    //      rotary_position_interpolation_factor [1] on cpu, float, optional
    ...
    // RoPE fails to generalize when it sees sequences at inference time which are longer than what was observed
    // during pretraining. We apply a linear scaling factor to the RoPE embeddings to counteract this. Our approach
    // is similar to that described in the Meta paper https://arxiv.org/abs/2306.15595
    const float rotary_position_interpolation_factor = \
        input_tensors->find("rotary_position_interpolation_factor") != input_tensors->end()
    ...
    if (has_prefix_prompt_ || has_prefix_soft_prompt_ || max_input_length > 1) {
        ...
        if (std::getenv("BENCHMARK_MEASURE_CEL")) {
            mpi::barrier();
        ...
        if (std::getenv("BENCHMARK_MEASURE_CEL")) {
            ...
            printf("INFO: Context-Encoding-Latency is %.2f ms\n",
                   ((context_end.tv_sec - context_start.tv_sec) * 1000 + (context_end.tv_usec - context_start.tv_usec) * 0.001));
        }
        sync_check_cuda_error();
    ...
    for (int step = max_input_length; step < (int)max_output_seq_len; step++) {
        ...
        *generation_should_stop_   = true;
        ...
        if (pipeline_para_.world_size_ > 1) {
            ...
            ftNcclBroadCast(finished_buf_, batch_size * beam_width, pipeline_para_.world_size_ - 1, pipeline_para_, stream_);

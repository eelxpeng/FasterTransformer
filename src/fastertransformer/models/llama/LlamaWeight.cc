    ...
    for (int l = 0; l < num_layer_; l++) {
        if (isValidLayerParallelId(l)) {
            decoder_layer_weights.push_back(new LlamaDecoderLayerWeight<T>(
                head_num, kv_head_num, size_per_head, inter_size_, tensor_para_size_, tensor_para_rank_, use_gptj_residual_));
        }
        else {
...
LlamaWeight<T>::~LlamaWeight()
{
    if (is_maintain_buffer == true) {
        for (int i = 0; i < weights_ptr.size(); i++) {
            // weights_ptr[3] == lm_head will have been freed already
            if (i == 3 && shared_embed_ && weights_ptr[i] == nullptr) {
                continue;
            }
...
template<typename T>
void LlamaWeight<T>::clearLMHeadWeights()
{
    deviceFree(weights_ptr[3]);
    weights_ptr[3]                = nullptr;
    post_decoder_embedding.kernel = weights_ptr[0];
}

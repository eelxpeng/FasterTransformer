...
{
    if (shape.size() > 2) {
        FT_LOG_ERROR("[ERROR] shape should have less than two dims \n");
        return std::vector<T>();
    }
    ...
    if (size == 0) {
        FT_LOG_ERROR("shape is zero, skip loading weight from file %s \n", filename.c_str());
        return std::vector<T>();
    }
    ...
    if (!in.is_open()) {
        FT_LOG_ERROR("file %s cannot be opened, loading model fails! \n", filename.c_str());
        return std::vector<T>();
    }
    ...
    if (in_get_size != loaded_data_size) {
        FT_LOG_ERROR("file %s only has %ld, but request %ld, loading model fails! \n",
                       filename.c_str(),
                       in_get_size,
                       loaded_data_size);
        return std::vector<T>();

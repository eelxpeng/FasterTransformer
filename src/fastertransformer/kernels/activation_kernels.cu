...
struct GeluActivation {
    ...
    {
        // TODO REMOVE (float) AFTER WE PATCH PACKING ISSUE
        const float cdf = 0.5f * (1.0f + tanh_opt((0.7978845608028654f * ((float)val + 0.044715f * (float)val * (float)val * (float)val))));
        return (float)val * cdf;
    }
};
...
void invokeGenericActivation(T*           out,
                             ...
                             cudaStream_t stream)
{
    // TODO root cause packing bug and revert this change

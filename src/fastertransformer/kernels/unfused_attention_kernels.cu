...
template <typename T>
__device__ T get_bucket_for_relative_position(const int relative_position, const int num_buckets, const int max_distance,
                                              const T* rel_attn_bias_table_smem_) {
    int max_exact                   = num_buckets / 2;
    bool is_small                   = relative_position < max_exact;

    int relative_position_if_large  = max_exact + (int) (logf(relative_position * 1.0f / max_exact) / logf((float) max_distance / max_exact)
                    * (num_buckets - max_exact));
    relative_position_if_large      = min(relative_position_if_large, num_buckets - 1);

    int bucket_idx                  = is_small ? relative_position : relative_position_if_large;
    return rel_attn_bias_table_smem_[bucket_idx];
}

template <typename T>
__device__ void construct_rel_bias_smem(const int load_num_threads, const int head_idx, const int num_buckets,
                                   const T* relative_attn_bias_table, T* rel_attn_bias_table_smem_) {
    for (int bucket_idx = threadIdx.x; bucket_idx < num_buckets; bucket_idx+=load_num_threads) {
        rel_attn_bias_table_smem_[bucket_idx] = relative_attn_bias_table[head_idx * num_buckets + bucket_idx];
    }
    // Sync threads to ensure smem write is complete before some other part of code accesses it.
    __syncthreads();
}
...
__global__ void softmax_kernel(T*          attn_score,
                               ...
                               const T* relative_attn_bias_table,
                               ...
                               const int max_distance)
{
    ...
    // Dynamic Allocation of shared memory and cast it to float*
    extern __shared__ char smem_[];
    T* rel_attn_bias_table_smem_ = reinterpret_cast<T*>(smem_);

    if (relative_attn_bias_table != nullptr && num_buckets > 0) {
        construct_rel_bias_smem(min(blockDim.x, num_buckets), hi, num_buckets, relative_attn_bias_table, rel_attn_bias_table_smem_);
    }
    ...
    for (int qi = blockIdx.x; qi < q_length; qi += gridDim.x) {
        ...
        for (int i = 0; blockDim.x * i + threadIdx.x < k_length; i++) {
            ...
            float rel_attn_bias = 0.0f;
            if (relative_attn_bias_table != nullptr) {
	            rel_attn_bias = static_cast<float>(get_bucket_for_relative_position(abs(ki - qi),
	                            num_buckets, max_distance, rel_attn_bias_table_smem_));
            }
            ...
            data[i]   = qk_scale * qk_val + rel_attn_bias + qk_bias;
                                  ...
                                  const int max_distance)
{
    ...
    // Dynamic shared memory for Relative attn bias table.
    // The size of this is computed before the kernel call and is provided in the template
    extern __shared__ char smem_[];
    T* rel_attn_bias_table_smem_ = reinterpret_cast<T*>(smem_);

    // Load the relative attention bias table into shared memory for on the fly computation.
    if (relative_attn_bias_table != nullptr && num_buckets > 0) {
        construct_rel_bias_smem(min(blockDim.x, num_buckets), hi, num_buckets, relative_attn_bias_table, rel_attn_bias_table_smem_);
    }
    ...
    for (int qi = blockIdx.x; qi < q_length; qi += gridDim.x) {
        ...
        for (int i = 0; blockDim.x * i + threadIdx.x < (k_length / 2) && i < ITEMS_PER_THREAD; i++) {
            ...
            T rel_attn_bias[2];
            T2 rel_attn_bias_h2 = ZERO;
            if (relative_attn_bias_table != nullptr) {
                // For T2 vectorization, we should handle every two elements along with k-dim simultaneously. To do this,
                // we process 2*ki and 2*ki+1 at ones.
#pragma unroll
                for (int idx=0; idx<2; idx++) {
                    rel_attn_bias[idx] = get_bucket_for_relative_position(abs(2 * ki + idx - qi),
                              num_buckets, max_distance, rel_attn_bias_table_smem_);
                }
                // construct h2
                rel_attn_bias_h2 = T2(rel_attn_bias[0], rel_attn_bias[1]);
            }
...
#pragma unroll
            ...
            T2 rel_attn_bias_h2[Q_ITEMS_PER_THREAD];
            T rel_attn_bias[2];
            if (relative_attn_bias_table != nullptr) {
#pragma unroll
                for (int j=0; j < Q_ITEMS; j++) {
...
#pragma unroll
                    ...
                    // we process 2*ki and 2*ki+1 at ones.
                    for (int idx=0; idx<2; idx++) {
                        rel_attn_bias[idx] = get_bucket_for_relative_position(abs(2 * ki + idx - qidx),
                        	                            num_buckets, max_distance, rel_attn_bias_table_smem_);
                    }
                    // construct h2
                    rel_attn_bias_h2[j] = T2(rel_attn_bias[0], rel_attn_bias[1]);
                }
            }
#pragma unroll
            for (int j = 0; j < Q_ITEMS; j++) {
...
#pragma unroll
            for (int j = 0; j < Q_ITEMS; j++) {
                ...
                if (relative_attn_bias_table != nullptr) {
                    val = hadd2<T2>(val, rel_attn_bias_h2[j]);
                }
    ...
    if (is_half2) {                                                                                                    \
        if (grid.x % 4 == 0) {                                                                                         \
            ...
            softmax_kernel_h2_v2<T_, ITEMS_PER_THREAD, 4>                                                              \
                <<<grid, block, smem_sz, stream>>>((T_*)param.attention_score,                                         \
                                             (const T_*)param.qk,                                                      \
                                             ...
                                             (const T_*) param.relative_attn_bias_table,                               \
                                             param.num_buckets,                                                        \
                                             param.max_distance);                                                      \
        }                                                                                                              \
        else {                                                                                                         \
            softmax_kernel_h2<T_, ITEMS_PER_THREAD><<<grid, block, smem_sz, stream>>>((T_*)param.attention_score,      \
                                                                                (const T_*)param.qk,                   \
                                                                                (const T_*)param.attention_mask,       \
                                                                                (const T_*)param.linear_bias_slopes,   \
                                                                                param.batch_size,                      \
        ...
        softmax_kernel<T, T_IN, ITEMS_PER_THREAD><<<grid, block, smem_sz, stream>>>(param.attention_score,             \
                                                                              param.qk,                                \
                                                                              ...
                                                                              param.k_length,                          \
                                                                              param.qk_scale,                          \
                                                                              param.relative_attn_bias_table,          \
                                                                              param.num_buckets,                       \
                                                                              param.max_distance);                     \
    }
...
void invokeMaskedSoftmax(MaskedSoftmaxParam<T, T_IN>& param, cudaStream_t stream)
{
    ...
    // relative_attn_bias  (head_num, num_buckets_or_max_seq_len)
    // num_buckets
    // max_distance
    ...
    size_t smem_sz = 0;
    if (param.relative_attn_bias_table != nullptr && param.num_buckets > 0) {
        smem_sz = param.num_buckets * sizeof(T);
    }

    if (block.x > 131072) {
        throw std::runtime_error(std::string("[FT][ERROR] Sequence length has exceeded limit of 131072 and softmax kernel will not launch: ") + "\n");
    }
    else if (block.x > 65536 && block.x <= 131072) {
        LAUNCH_MAKSED_SOFTMAX(128)
    }
    else if (block.x > 32768 && block.x <= 65536) {
        LAUNCH_MAKSED_SOFTMAX(64)
    }
    else if (block.x > 16384 && block.x <= 32768) {
        LAUNCH_MAKSED_SOFTMAX(32)
    }
    else if (block.x > 8192 && block.x <= 16384) {
        LAUNCH_MAKSED_SOFTMAX(16)
    }
    else if (block.x > 4096 && block.x <= 8192) {
...
void invokeMaskedSoftmax(MaskedSoftmaxParam<__nv_bfloat16, float>& param, cudaStream_t stream)
{
    ...
    if (block.x > 131072) {
        ...
        LAUNCH_MAKSED_SOFTMAX_(__nv_bfloat16, 128);
    }
    else if (block.x > 32768 && block.x <= 65536) {
        LAUNCH_MAKSED_SOFTMAX_(__nv_bfloat16, 64);
    }
    else if (block.x > 16384 && block.x <= 32768) {
        LAUNCH_MAKSED_SOFTMAX_(__nv_bfloat16, 32);
    }
    else if (block.x > 8192 && block.x <= 16384) {
        LAUNCH_MAKSED_SOFTMAX_(__nv_bfloat16, 16);
    }
    else if (block.x > 4096 && block.x <= 8192) {
                                                   ...
                                                   const float* scale)
{
    ...
    const int  n          = head_num * size_per_head;
    const bool i8_input   = scale != nullptr;
                                    ...
                                    cudaStream_t                     stream)
{
    ...
    if (rotary_embedding_dim == 0 && param.max_prefix_prompt_length == 0) {
        ...
        FT_CHECK_WITH_INFO(scale == nullptr, "w8a8 not yet implemented with prefix prompt");  // TODO(mseznec)
...
__global__ void addRelativeAttentionBiasUnaligned(T*        qk_buf,
                                                  const BT* relative_attention_bias,
                                                  const int batch_size,
                                                  const int head_num,
                                                  const int seq_len,
                                                  int       max_seq_len)
{
    ...
    for (int i = threadIdx.x; i < seq_len * seq_len; i += blockDim.x) {
        int seq_i = i / seq_len;
        int seq_j = i % seq_len;
        ...
        qk_buf[qk_index] = (T)add(relative_attention_bias[bias_index], qk_buf[qk_index]);
    }
}

template<typename T, typename BT>
void invokeAddRelativeAttentionBiasUnaligned(T*           qk_buf,
                                             const BT*    relative_attention_bias,
                                             const int    batch_size,
                                             const int    head_num,
                                             const int    seq_len,
                                             ...
                                             cudaStream_t stream)
{
    ...
    addRelativeAttentionBiasUnaligned<<<grid, block, 0, stream>>>(
        qk_buf, relative_attention_bias, batch_size, head_num, seq_len, max_seq_len);
}

#define INSTANTIATEADDRELATIVEATTENTIONBIASUNALIGNED(T, BT)                                                            \
    template void invokeAddRelativeAttentionBiasUnaligned(T*           qk_buf,                                         \
                                                          const BT*    relative_attention_bias,                        \
                                                          const int    batch_size,                                     \
                                                          const int    head_num,                                       \
                                                          const int    seq_len,                                        \
                                                          const int    max_seq_len,                                    \
                                                          cudaStream_t stream)
INSTANTIATEADDRELATIVEATTENTIONBIASUNALIGNED(float, float);
INSTANTIATEADDRELATIVEATTENTIONBIASUNALIGNED(half, half);
#ifdef ENABLE_BF16
INSTANTIATEADDRELATIVEATTENTIONBIASUNALIGNED(__nv_bfloat16, __nv_bfloat16);
INSTANTIATEADDRELATIVEATTENTIONBIASUNALIGNED(float, __nv_bfloat16);
#endif
#undef INSTANTIATEADDRELATIVEATTENTIONBIASUNALIGNED

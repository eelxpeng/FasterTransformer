...
#define MMHA_LAUNCH_KERNEL(                                                                                            \
    ...
    cudaFuncSetAttribute(mmha::masked_multihead_attention_kernel<T,                                                    \
                                        Dh,                                                                            \
                                        Dh_MAX,                                                                        \
                                        THDS_PER_KEY,                                                                  \
                                        THDS_PER_VALUE,                                                                \
                                        THDS_PER_BLOCK,                                                                \
                                        DO_CROSS_ATTENTION,                                                            \
                                        HAS_BEAMS>, cudaFuncAttributeMaxDynamicSharedMemorySize, smem_sz);             \
...
template void mmha_launch_kernel<float, 104, 128, Masked_multihead_attention_params<float>>(
    const Masked_multihead_attention_params<float>& params, const cudaStream_t& stream);
template void mmha_launch_kernel<uint16_t, 104, 128, Masked_multihead_attention_params<uint16_t>>(
    const Masked_multihead_attention_params<uint16_t>& params, const cudaStream_t& stream);
#ifdef ENABLE_BF16
template void mmha_launch_kernel<__nv_bfloat16, 104, 128, Masked_multihead_attention_params<__nv_bfloat16>>(
    const Masked_multihead_attention_params<__nv_bfloat16>& params, const cudaStream_t& stream);
#endif
#ifdef ENABLE_FP8
template void mmha_launch_kernel<__nv_fp8_e4m3, 104, 128, Masked_multihead_attention_params<__nv_fp8_e4m3>>(
    const Masked_multihead_attention_params<__nv_fp8_e4m3>& params, const cudaStream_t& stream);
#endif

template void mmha_launch_kernel<float, 104, 128, Cross_multihead_attention_params<float>>(
    const Cross_multihead_attention_params<float>& params, const cudaStream_t& stream);
template void mmha_launch_kernel<uint16_t, 104, 128, Cross_multihead_attention_params<uint16_t>>(
    const Cross_multihead_attention_params<uint16_t>& params, const cudaStream_t& stream);
#ifdef ENABLE_BF16
template void mmha_launch_kernel<__nv_bfloat16, 104, 128, Cross_multihead_attention_params<__nv_bfloat16>>(
    const Cross_multihead_attention_params<__nv_bfloat16>& params, const cudaStream_t& stream);
#endif
#ifdef ENABLE_FP8
template void mmha_launch_kernel<__nv_fp8_e4m3, 104, 128, Cross_multihead_attention_params<__nv_fp8_e4m3>>(

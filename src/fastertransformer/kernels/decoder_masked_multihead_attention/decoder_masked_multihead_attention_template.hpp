                                 ...
                                 int                                                      threads_per_block)
{
    ...
    //Add relative bias buffer at the front. If zero we wont need the buffer.
    //Typical size of this buffer should be 64 bytes for BF16 using 32 buckets - so small footprint
    //If we start running models with very large buckets, we need to consider reusing existing shared mem buffers.
    size_t relative_bias_size = params.num_bucket * sizeof(T);

    // The max.
    return relative_bias_size + max(max(softmax_sz, red_sz), transpose_rotary_size) ;
}
...
// NOTE: If we wish to fuse relative position embedding bucket lookups into the kernel (which would be strictly faster)
// we should pass params.relative_attention_bias as the buckets weight table of shape (local heads, num_bucket) AND
// params.num_bucket and params.max_distance. To use a precomputed table, pass in params.relative_attention_bias as
// the fully constructed bias table of (1,Local Heads, Max Seq Len, Max Seq Len) - do not set num_bucket
template<
...
__global__ void masked_multihead_attention_kernel(Multihead_attention_params<T, DO_CROSS_ATTENTION> params)
{
    ...
    // The number of bytes reserved for storing per head relative bias bucket weights
    // Since we reserve this buffer at the beginning, all other buffer starts will be offset by this amount
    // If we don't use relative bias, it is as if we never allocated a buffer.
    const int rel_bias_bytes = params.num_bucket * sizeof(T);
    char* rel_bias_smem_ = smem_;

    // The shared memory for the Q*K^T values and partial logits in softmax.
    float* qk_smem = reinterpret_cast<float*>(smem_ + rel_bias_bytes);

    // The shared memory for the logits. For FP32, that's the same buffer as qk_smem.
    char* logits_smem_ = smem_ + rel_bias_bytes;
#ifndef MMHA_USE_FP32_ACUM_FOR_LOGITS
    if (sizeof(Tk) != 4) {
    ...
    Tk* out_smem = reinterpret_cast<Tk*>(smem_ + rel_bias_bytes);
    ...
    const float position_value = (params.timestep - padd_len) / params.rotary_position_interpolation_factor;
...
#pragma unroll
    ...
    int num_bucket   = params.num_bucket;
    int max_distance = params.max_distance;

    T* smem_bias_buckets = reinterpret_cast<T*>(rel_bias_smem_);

    // Perform Shared Memory load only for on the fly Relative Attention Bias computation
    // This shared mem load speed up reads significantly since we read the same 32 floats repeatedly N^2 times
    if (params.relative_attention_bias != nullptr && num_bucket > 0) {
        // Tiled Load of Relative Bias Buckets into shared memory - Should we do this well ahead of time?

        // If num_bucket is 'too' large we will run out of smem - We are going to assume here it's very small
        const int load_num_threads = min(THREADS_PER_BLOCK, num_bucket);  //

        // The loop handles the case where we have a larger number of buckets than threads per block avlbl to load
        // Most of the time, a thread would only ever do one iteration , but we follow generic pattern here.
        for (int bucket_idx = tidx; bucket_idx < num_bucket; bucket_idx += load_num_threads) {
            smem_bias_buckets[bucket_idx] = params.relative_attention_bias[hi * num_bucket + bucket_idx];
        }

        // We Must sync threads to ensure smem write is complete before accessing it anywhere after this point
        // Note - we can sync inside this conditional since all threads will either participate , or not if rel bias is not present
        __syncthreads();
    }
    ...
    if (tidx == 0) {
        ...
        if (params.relative_attention_bias != nullptr) {
            if(num_bucket > 0) {
                // We are on the lead diagonal here where relative position is zero -
                // Simply pick the first bucket without doing the relative bucket mapping computation
                qk = add(qk, smem_bias_buckets[0]);
            } else {
                qk = add(qk,
...
#pragma unroll
        ...
        if (ti < tlength && tidx % THREADS_PER_KEY == 0) {
            if (params.relative_attention_bias != nullptr) {
                if(num_bucket > 0) {
                    // We do an 'on the fly' relative position to bucket mapping here i.e.
                    // fusing buildRelativeAttentionBias Note that we consider the 'positive' value of relative
                    // position here to index into bucket.

                    // Ignore padding tokens for relative position if paying attention to the prompt
                    int relative_position = 0;

                    // Currently an input token
                    if (ti < params.max_input_length && ti < (tlength-padd_len)) {
                        relative_position = (tlength-padd_len) - ti;
                    }
                    else {
                        relative_position = tlength- ti;
                    }
                    ...
                    relative_position_if_large = min(relative_position_if_large, num_bucket - 1);
                    int relative_buckets       = is_small ? relative_position : relative_position_if_large;
                    qk = add(qk, smem_bias_buckets[relative_buckets]);
                } else {
                    qk = add(qk,
                             params.relative_attention_bias[hi * params.relative_attention_bias_stride
                                                                * params.relative_attention_bias_stride

...
#include "src/fastertransformer/th_op/agm/AgmDecodingOp.h"
...
template<typename T>
FTAgmDecoding<T>::FTAgmDecoding(int64_t                        head_num,
    ...
    _weights(w)
{
    ...
    decoding_weights.setAgmStructureDiff(agm_with_bias, use_gated_activation, position_embedding_type);
    ...
    for (int i = 0; i < layer_num_; ++i) {
        ...
        if (agm_with_bias_) {
            ...
            decoder_layer_weights->self_attention_weights.query_weight.bias = nullptr;
                // get_ptr<T>(_weights[17]) + layer_index * 3 * hidden_dim / world_size; // must use null since with_bias is overall true but no qkv bias -- should hard set to null
            decoder_layer_weights->self_attention_weights.attention_output_weight.bias = nullptr;
            ...
            // per layer
            decoder_layer_weights->absolute_or_relative_position_embedding = get_ptr<T>(_weights[15]) + layer_index * head_num_ * num_bucket_ /* / world_size*/;
        }
...
template<typename T>
std::vector<th::Tensor> FTAgmDecoding<T>::forward(th::optional<int64_t>    beam_width_opt,
                                                  size_t                   max_seq_len,
                                                  ...
                                                  th::optional<th::Tensor> no_repeat_ngram_size_opt,
                                                  th::optional<th::Tensor> forced_decoder_ids_opt)
{
    // input validation
    ...
    ft::AgmDecoding<T> decoding = ft::AgmDecoding<T>(
        batch_size,
        max_seq_len,
        mem_max_seq_len,
        beam_width,
    ...
    if (no_repeat_ngram_size_opt.has_value()) {
        input_tensors.insert({"no_repeat_ngram_size", convert_tensor<int>(no_repeat_ngram_size_opt.value())});
    }
    ...
    if (forced_decoder_ids_opt.has_value()) {
        input_tensors.insert({"forced_decoder_ids", convert_tensor<int>(forced_decoder_ids_opt.value())});
    }
...
template class FTAgmDecoding<float>;
template class FTAgmDecoding<half>;
#ifdef ENABLE_BF16
template class FTAgmDecoding<__nv_bfloat16>;
#endif

FasterTransformerAgmDecoding::FasterTransformerAgmDecoding(int64_t              head_num,
                                                           int64_t              size_per_head,
                                                           int64_t              inter_size,
                                                           int64_t              mem_d_model,
                                                           int64_t              d_model,
            ...
            after_ffn_adapter_layernorm_beta}
{
    ...
    switch (_st) {
        case at::ScalarType::Float:
            ftdecoding = new torch_ext::FTAgmDecoding<float>(head_num,
                                                             size_per_head,
                                                             inter_size,
                                                             mem_d_model,
                                                             d_model,
        ...
        case at::ScalarType::Half:
            ftdecoding = new torch_ext::FTAgmDecoding<half>(head_num,
                                                            size_per_head,
                                                            inter_size,
                                                            mem_d_model,
                                                            d_model,
        ...
        case at::ScalarType::BFloat16:
            ftdecoding = new torch_ext::FTAgmDecoding<__nv_bfloat16>(head_num,
                                                                     size_per_head,
                                                                     inter_size,
                                                                     mem_d_model,
                                                                     d_model,
...
FasterTransformerAgmDecoding::~FasterTransformerAgmDecoding()
{
    delete ftdecoding;
}

std::vector<th::Tensor> FasterTransformerAgmDecoding::forward(th::optional<int64_t>    beam_width,
                                                              int64_t                  max_seq_len,
                                                              ...
                                                              th::optional<th::Tensor> no_repeat_ngram_size,
                                                              th::optional<th::Tensor> forced_decoder_ids)
{
...
std::vector<th::Tensor> FasterTransformerAgmDecoding::get_pickle_info() const
{
...
static auto fasterTransformerAgmDecodingTHS =
#ifdef LEGACY_THS
    torch::jit::class_<torch_ext::FasterTransformerAgmDecoding>("FasterTransformerAgmDecoding")
#else
    torch::jit::class_<torch_ext::FasterTransformerAgmDecoding>("FasterTransformer", "AgmDecoding")
#endif
        .def(torch::jit::init<int64_t,
                              ...
                              th::Tensor,
                              th::Tensor>())
        .def("forward", &torch_ext::FasterTransformerAgmDecoding::forward);

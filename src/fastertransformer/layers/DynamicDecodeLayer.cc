...
void DynamicDecodeLayer<T>::forward(TensorMap* output_tensors, TensorMap* input_tensors)
{
    ...
    if (input_tensors->isExist("no_repeat_ngram_size")) {
        const int* no_repeat_ngram_size_buf = input_tensors->at("no_repeat_ngram_size").getPtr<const int>();
        const int  batch_id_offset          = ite * local_batch_size;

        FT_CHECK_WITH_INFO(input_tensors->at("no_repeat_ngram_size").shape[0] == batch_size,
                           "No-repeat Ngram dimension must be equal to batch size");
        ...
        invokeBanRepeatNgram((T*)input_tensors->at("logits").getPtrWithOffset(decode_vocab_size_units_offset),
                             ...
                             local_batch_size,
                             beam_width,
                             no_repeat_ngram_size_buf + batch_id_offset,  // offset the ngram buffer correctly
                             id_offset,
                             vocab_size_padded_,
                             step,
                             stream_);

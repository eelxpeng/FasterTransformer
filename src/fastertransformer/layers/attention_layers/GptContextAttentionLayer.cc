                                          ...
                                          const AttentionWeight<T>* attention_weights)
{
    ...
    //      relative_attn_bias_table [local_head_num, num_bucket_or_max_seq_len], on gpu (optional)
    ...
    const T* relative_attn_bias_table       = input_tensors->getPtr<T>("relative_attn_bias_table", nullptr);
    const bool has_relative_attention_bias = input_tensors->isExist("relative_attn_bias_table");
    const int num_buckets                   = input_tensors->getVal<int>("num_buckets", 0);
    const int max_distance                  = input_tensors->getVal<int>("max_distance", 0);
    ...
    if (is_final == false) {
        ...
        // const T              qk_scale            = static_cast<T>(1.0f / sqrtf(size_per_head_ * 1.0f) * q_scaling_);
        const T qk_scale = has_relative_attention_bias ? static_cast<T>(1.0f) : static_cast<T>(1.0f / sqrtf(size_per_head_ * 1.0f) * q_scaling_);
        ...
        if (attention_type != AttentionType::FUSED_MHA) {
            if (is_qk_buf_float_ == true && gemm_data_type != CUDA_R_32F) {
                ...
                param.relative_attn_bias_table  = const_cast<T*>(relative_attn_bias_table);     // (head_num, num_buckets_or_max_seq_len)
                param.num_buckets               = num_buckets;
                ...
                cublas_wrapper_->stridedBatchedGemm(
                    ...
                    has_relative_attention_bias ? 1.0f / (sqrtf((float)size_per_head_) * q_scaling_) : 1.0);  // q_scaling apply here!

                // since we now want softmax[scale(Q)*K + rel pos bias] here, q_scaling can't be applied during softmax
                // phase by qk_scale! must set qk_scale=1.0 and do scaling before adding rel pos bias in the softmax kernel
                POP_RANGE;
                PUSH_RANGE("softmax");
                ...
                param.qk_scale                  = qk_scale;
                param.linear_bias_slopes        = const_cast<T*>(linear_bias_slopes);  // (head_num,), optional
                param.relative_attn_bias_table  = const_cast<T*>(relative_attn_bias_table);
                param.num_buckets               = num_buckets;
                param.max_distance              = max_distance;
                invokeMaskedSoftmax(param, stream_);
                sync_check_cuda_error();

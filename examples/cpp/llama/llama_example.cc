...
template<typename T>
void llama_example(const INIReader reader, std::string start_ids_csv);
...
int main(int argc, char* argv[])
{
    ...
    std::string ini_name;
    std::string start_ids_csv;
    if (argc == 3) {
        ini_name = std::string(argv[1]);
        start_ids_csv = std::string(argv[2]);
    } else if (argc == 2) {
        ini_name = std::string(argv[1]);
        start_ids_csv = "../examples/cpp/llama/start_ids.csv";
    } else {
    ...
    if (data_type == "fp32") {
        llama_example<float>(reader, start_ids_csv);
    }
    else if (data_type == "fp16") {
        llama_example<half>(reader, start_ids_csv);
    }
#ifdef ENABLE_BF16
    else if (data_type == "bf16") {
        llama_example<__nv_bfloat16>(reader, start_ids_csv);
    }
#endif
    else {
...
void llama_example(const INIReader reader, std::string start_ids_csv)
{
    ...
    const float  rotary_position_interpolation_factor = reader.GetFloat(model_name, "rotary_position_interpolation_factor", 1.f);
    ...
    if (file_exists_test("/opt/amazon/alexa_triton_inference_engine/AlexaFasterTransformer/examples/cpp/llama/bad_words.csv")) {
        read_word_list("/opt/amazon/alexa_triton_inference_engine/AlexaFasterTransformer/examples/cpp/llama/bad_words.csv", bad_words);
    }
    ...
    if (file_exists_test("/opt/amazon/alexa_triton_inference_engine/AlexaFasterTransformer/examples/cpp/llama/stop_words.csv")) {
        read_word_list("/opt/amazon/alexa_triton_inference_engine/AlexaFasterTransformer/examples/cpp/llama/stop_words.csv", stop_words);
    }
    ...
    if (file_exists_test(model_dir)) {
        gpt_weights.loadModel(model_dir);
    }
    else {
        gpt_weights.clearLMHeadWeights();
    }

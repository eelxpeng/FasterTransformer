...
template<typename T>
void gptj_example(const INIReader reader, std::string start_ids_csv);
...
int main(int argc, char* argv[])
{
    ...
    if (argc == 3) {
        ...
        ini_name = std::string(argv[1]);
        start_ids_csv = "../examples/cpp/gptj/start_ids.csv";
    } else {
    ...
    if (data_type == "fp32") {
        gptj_example<float>(reader, start_ids_csv);
    }
    else if (data_type == "fp16") {
        gptj_example<half>(reader, start_ids_csv);
    }
#ifdef ENABLE_BF16
    else if (data_type == "bf16") {
        gptj_example<__nv_bfloat16>(reader, start_ids_csv);
    }
#endif
    else {
